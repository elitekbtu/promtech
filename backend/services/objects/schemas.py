...
class WaterObjectBase(BaseModel):
    ...
    name: str = Field(
        ..., min_length=1, max_length=255, description="Water object name"
    )
    region: str = Field(
        ..., min_length=1, max_length=255, description="Administrative region"
    )
    ...
    passport_date: Optional[datetime] = Field(
        None, description="Date of last passport inspection"
    )
    technical_condition: int = Field(
        ..., ge=1, le=5, description="Technical condition score (1=worst, 5=best)"
    )
    ...
    pdf_url: Optional[str] = Field(
        None, max_length=500, description="URL to passport PDF document"
    )
...
class WaterObjectGuestResponse(BaseModel):
    ...
    technical_condition: Optional[int]
...
class WaterObjectFilter(BaseModel):
    ...
    search: Optional[str] = Field(None, description="Search by name or region (case-insensitive)")
    ...
    min_technical_condition: Optional[int] = Field(
        None, ge=1, le=5, description="Minimum technical condition"
    )
    max_technical_condition: Optional[int] = Field(
        None, ge=1, le=5, description="Maximum technical condition"
    )
    min_priority: Optional[int] = Field(
        None, ge=0, description="Minimum priority score"
    )
    max_priority: Optional[int] = Field(
        None, ge=0, description="Maximum priority score"
    )
    priority_level: Optional[PriorityLevel] = Field(
        None, description="Filter by priority level"
    )
    passport_date_from: Optional[datetime] = Field(
        None, description="Passport date from"
    )
...
class PaginationParams(BaseModel):
    ...
    limit: int = Field(
        100, gt=0, le=100, description="Maximum number of items to return"
    )
